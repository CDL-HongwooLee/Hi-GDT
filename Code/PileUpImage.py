...
Default = 'SCALE'.
    ...
    '''Use this flag to skip generating matrix.pickle file from a .hic file. 
...
if set, -i {matrix.pickle} option is required for input file.
    ...
    '''Path to the input matrix.pickle file.
...
def MakeIFpickleFiles(outdir, prefix, chrStartFragNumDic, chromlist, res, bf, t):
    ...
    for chrom in chromlist:
        with open(f'{outdir}/tmp/{prefix}.{chrom}_{chrom}.{res}{bf}.matrix.pickle', 'rb') as pk:
            mergedDic[chrom] = pickle.load(pk)
    with open(f'{outdir}/{prefix}.{res}{bf}.matrix.pickle','wb') as pko:
        pickle.dump(mergedDic, pko)
    sp.run(f'rm {outdir}/tmp/{prefix}.*_*.{res}{bf}.matrix.pickle {outdir}/tmp/{prefix}.*_*.{res}{bf}.txt', shell=True)
    return mergedDic
...
def ArrayParallelizing(outdir, prefix, Gene_minMaxFragDic, RElengthDic, bs, imgsize, chrom, IFdic):
    ...
    with open(f'{outdir}/tmp/{prefix}.region_addBoundary.{chrom}_{chrom}.txt') as r1:
        ...
        with open(f'{outdir}/tmp/{prefix}.{chrom}_{chrom}.pickle','wb') as p2:
...
def mergePickle(outdir, prefix):
    ...
    outdict = dict()
    for chrom in chromlist:
        with open(f'{outdir}/tmp/{prefix}.{chrom}_{chrom}.pickle', 'rb') as pk:
            genepk = pickle.load(pk)
            outdict.update(genepk)
        sp.call(f'rm {outdir}/tmp/{prefix}.{chrom}_{chrom}.pickle', shell=True, universal_newlines=True)
    with open(f'{outdir}/{prefix}.pickle','wb') as p2:
...
if __name__ == '__main__':
    ...
    #chromlist = sp.check_output(f'less {args.chromSize} | cut -f 1', shell=True, universal_newlines=True).rstrip().split('\n')
    ...
    if args.skip == False:
        ...
        print('\nMake interaction matrix.pickle.....\n')
        start_time = time.time()
        #chromIFdic = makeIFdic(args.outdir, args.prefix, args.refile, chromlist, args.resolution, args.threads)
        ...
        #with open(args.infile, 'rb') as pI:
        #    chromIFdic = pickle.load(pI)
